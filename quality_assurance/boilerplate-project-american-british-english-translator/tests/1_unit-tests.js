...
suite('Unit Tests', () => {
	...
	test('Translate Like a high tech Rube Goldberg machine. to British English', function() {
		...
		assert.include(res.translation, '<span class="highlight">Heath Robinson device</span>');
	});
	...
	test('Translate Paracetamol takes up to an hour to work. to American English', function() {
		...
		assert.include(res.translation, '<span class="highlight">Tylenol</span>');
	});

	test('Translate First, caramelise the onions. to American English', function() {
		const res = translator.translate('First, caramelise the onions.', 'british-to-american');
		assert.equal(res.translation, 'First, <span class="highlight">caramelize</span> the onions.');
	});

	test('Translate I spent the bank holiday at the funfair. to American English', function() {
		const res = translator.translate('I spent the bank holiday at the funfair.', 'british-to-american');
		assert.include(res.translation, '<span class="highlight">public holiday</span>');
		assert.include(res.translation, '<span class="highlight">carnival</span>');
	});

	test('Translate I had a bicky then went to the chippy. to American English', function() {
		const res = translator.translate('I had a bicky then went to the chippy.', 'british-to-american');
		assert.include(res.translation, '<span class="highlight">cookie</span>');
		assert.include(res.translation, '<span class="highlight">fish-and-chip shop</span>');
	});
	...
	test("Translate I've just got bits and bobs in my bum bag. to American English", function() {
		...
		assert.include(res.translation, '<span class="highlight">odds and ends</span>');
		assert.include(res.translation, '<span class="highlight">fanny pack</span>');
	});
	...
	test('Translate The car boot sale at Boxted Airfield was called off. to American English', function() {
		...
		assert.include(res.translation, '<span class="highlight">swap meet</span>');
	});
	...
	test('Highlight translation in Mangoes are my favorite fruit.', function() {
		const res = translator.translate('Mangoes are my favorite fruit.', 'american-to-british');
		assert.include(res.translation, '<span class="highlight">favourite</span>');
	});

	test('Highlight translation in I ate yogurt for breakfast.', function() {
		const res = translator.translate('I ate yogurt for breakfast.', 'american-to-british');
		assert.include(res.translation, '<span class="highlight">yoghurt</span>');
	});

	test('Highlight translation in We watched the footie match for a while.', function() {
		const res = translator.translate('We watched the footie match for a while.', 'british-to-american');
		assert.include(res.translation, '<span class="highlight">soccer</span>');
	});
